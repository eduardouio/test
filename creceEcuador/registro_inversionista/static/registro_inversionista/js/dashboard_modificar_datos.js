...
function cambiarHabilitadoInputs() {
  ...
  if (usuarioConfirmado == 1) {
    nonEditableInputs = [
      ...
      "modificar_apellidos",
      "modificar_cedula",
      "modificar_ruc_profesional_independiente",
      "modificar_titular",
    ];
  } else {
  ...
  if (datosDesdeBaseModificar) {
    $(".crece-modificar-datos-formulario-wrapper-boton-subir-doc").each(
      function () {
...
function cargarContratos() {
  $.ajax({
    type: "GET",
    url:
      "/inversionista/contratos/" +
      $("#perfil_ver_contratos").attr("data-id") +
      "/",
    success: function (result) {
      ...
      result.data.forEach(function (contrato) {
        stringContratos +=
          ...
          "                              <span>" +
          '                                <a target="_blank" rel="noopener noreferrer" onclick="return previewFile(\'' +
          contrato.contrato +
          "', '" +
          getNombreContrato(contrato.contrato) +
          '\')" href="' +
          contrato.contrato +
          '">' +
          getNombreContrato(contrato.contrato) +
...
function inicializarVerPerfil() {
  ...
  */
  ...
  $(
    ".crece-modificar-datos-formulario-wrapper-editar, div.crece-modificar-datos-formulario-wrapper-editar *"
  ).click(function () {
  ...
  hacerNoRequired(
    ".crece-modificar-datos-formulario-profesional-independiente input"
  );
  ...
  function limpiar_password() {
    ...
    let input_password_confirmar = document.getElementById(
      "id_confirmar_password"
    );
    let div_text_confirmar = document.getElementById(
      "crece-show-hide-confirmar-password"
    );
  ...
  */
  ...
  $("#modificar_selectTrabajo").change(function () {
    if (this.value == 1) {
      $(".crece-modificar-datos-formulario-relacion-dependencia").show();
      hacerRequired(
        ".crece-modificar-datos-formulario-relacion-dependencia input"
      );
...
function cargarFuenteDeIngresosModificar() {
  ...
  if (ingresos) {
    ...
    if (dictIngresos.ANIOS_RELACION_DEPENDENCIA) {
      ...
      $("#modificar_cargo_relacion_dependencia").val(
        dictIngresos.CARGO_RELACION_DEPENDENCIA
      );
      ...
      $("#modificar_ruc_profesional_independiente").val(
        dictIngresos.RUC_PROFESIONAL_INDEPENDIENTE
      );
      $("#modificar_actividad_profesional_independiente").val(
        dictIngresos.ACTIVIDAD_PROFESIONAL_INDEPENDIENTE
      );
      ...
      $("#modificar_actividad_auto_empleado").val(
        dictIngresos.ACTIVIDAD_AUTO_EMPLEADO
      );
    } else {
...
function obtenerRespuestasModificar() {
  ...
  var fuente_ingresos = $("#modificar_selectTrabajo")
    .children("option:selected")
    .val();
  ...
  if (fuente_ingresos == 1) {
    ...
    dictFuenteIngresos.cargo_relacion_dependencia = $(
      "#modificar_cargo_relacion_dependencia"
    ).val();
    //dictFuenteIngresos.anios_relacion_dependencia = $("#modificar_anios_relacion_dependencia").val();
  } else if (fuente_ingresos == 2) {
    dictFuenteIngresos.ruc_profesional_independiente = $(
      "#modificar_ruc_profesional_independiente"
    ).val();
    dictFuenteIngresos.actividad_profesional_independiente = $(
      "#modificar_actividad_profesional_independiente"
    ).val();
    ...
    dictFuenteIngresos.ruc_auto_empleado = $(
      "#modificar_ruc_auto_empleado"
    ).val();
    dictFuenteIngresos.actividad_auto_empleado = $(
      "#modificar_actividad_auto_empleado"
    ).val();
  }
  ...
  dictRespuestas.ingresos_mensuales = $(
    "#modificar_ingresos_aproximados"
  ).val();

  dictRespuestas.titular = $("#modificar_titular").val();
  dictRespuestas.banco = $("#modificar_selectBanco")
    .children("option:selected")
    .val();
  dictRespuestas.numero_cuenta = $("#modificar_numero_cuenta").val();
  dictRespuestas.tipo_cuenta = $("#modificar_selectTipoCuenta")
    .children("option:selected")
    .val();
...
function checkInputsModificar() {
  var es_valido = true;
  $(".crece-modificar-datos-formulario-wrapper input")
    .filter("[required]")
    .each(function () {
      if ($(this).val() === "") {
        ...
        if (
          this.name === "modificar_cedula" &&
          !validar_cedula_modificar(this)
        ) {
          focusAndInvalidate(this);
          $(".crece-modificar-datos-formulario-wrapper .error-modificar").html(
            "Ingrese su cédula correctamente"
          );
          $(
            ".crece-modificar-datos-formulario-wrapper .error-modificar-container"
          ).css("display", "flex");

          es_valido = false;
          return false;
        } else if (
          this.name === "modificar_cedula_conyugue" &&
          !validar_cedula_modificar(this)
        ) {
          focusAndInvalidate(this);

          $(".crece-modificar-datos-formulario-wrapper .error-modificar").html(
            "Ingrese la cédula de su cónyugue correctamente"
          );
          $(
            ".crece-modificar-datos-formulario-wrapper .error-modificar-container"
          ).css("display", "flex");
          ...
          this.name === "modificar_canton" &&
          !cantones.includes(this.value)
        ) {
          focusAndInvalidate(this);
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if ($(".crece-usuario-imagen.fa.fa-user-circle-o").length) {
        $.each(
          $(".crece-usuario-imagen.fa.fa-user-circle-o"),
          function (index, value) {
            value.outerHTML =
              `<img class="crece-usuario-imagen-seleccionada" src="` +
              response.data.ruta +
              `"></img>`;
          }
        );
      } else {
        $(".crece-usuario-imagen-seleccionada").attr(
          "src",
          response.data.ruta + "?t=" + new Date().getTime()
        );
        $(".crece-usuario-imagen-navbar").attr(
          "src",
          response.data.ruta + "?t=" + new Date().getTime()
        );
      }
      ...
      $(
        "#usuario_data_container *, .crece-usuario-imagen-seleccionada, .crece-usuario-imagen"
      ).click(function () {
    ...
    error: function () {
      $(".crece-modificar-datos-formulario-wrapper .error-modificar").html(
        "No se pudo cargar la imagen de perfil. Intente de nuevo."
      );
      $(
        ".crece-modificar-datos-formulario-wrapper .error-modificar-container"
      ).css("display", "flex");
  ...
  if (fase_inversion_actual != "FILL_INFO") {
    ...
    $(
      ".crece-flujo-inversionista-paso-cuatro, " +
        ".crece-flujo-inversionista-paso-cuatro span"
    )
      .prop("onclick", null)
      .off("click");
  } else {
    $.ajax({
      ...
      error: function () {
        $(".crece-modificar-datos-formulario-wrapper .error-modificar").html(
          "No se pudo cambiar el estado de la inversión."
        );
        $(
          ".crece-modificar-datos-formulario-wrapper .error-modificar-container"
        ).css("display", "flex");
